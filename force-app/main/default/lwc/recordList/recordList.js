...
const actions = [
  { label: "View", name: "view", iconName: "utility:view" },
...
export default class RecordList extends NavigationMixin(LightningElement) {
  ...
  getListViews({ data, error }) {
    if (error) {
      ...
      this.listViews =
        data.lists.map((l) => ({ value: l.id, label: l.label })) || [];
      this.currentListViewId = (
        data.lists.filter(
          (l) => l.apiName == this.filterName || l.id == this.filterName
        )[0] || data.lists[0]
      )?.id;
    }
  }
