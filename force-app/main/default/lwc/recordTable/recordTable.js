...
export default class RecordTable extends LightningDatatable {
  ...
  set objectInfo(objectInfo) {
    ...
    this.state.dataTypes = Object.values(objectInfo.fields).reduce(
      (dataTypes, { apiName, dataType }) => {
  ...
  convertData() {
    this.data =
      ...
      this.state.listUi.records.records.map((record) => {
        return Object.entries(record.fields).reduce(
          (recordData, [apiName, { value }]) => {
            recordData[apiName] =
  ...
  convertColumns() {
    this.columns = this.state.listUi && [
      ...this.state.listUi.info.displayColumns.map(
        ({ fieldApiName, label }) => ({
          label,
          fieldName: fieldApiName,
          type:
            fieldApiName === this.state.nameField
              ? "name"
