...
export default class Lwccreateclientobjectives extends LightningElement {
  ...
  refresh() {
    ...
    getUnusedObjectives({ clientId: this.recordId })
      ...
      .catch((error) => {
        this.error = error;
        console.error(
          `%crefresh(): REFRESH ERROR: ${JSON.stringify(error)}`,
          COLOR
        );
      });
  }
  ...
  handleClickArray(event) {
    ...
    if (this.selectedRows) {
      ...
      console.debug(
        `%chandleClickArray(): logging JSON: ${JSON.stringify(
          this.selectedRows
        )}`,
        COLOR
      );
      console.debug(
        `%chandleClickArray(): logging session: ${this.recordId}`,
        COLOR
      );
      console.debug(
        `%chandleClickArray(): imperative Call to createClientObjectivesByArray`,
        COLOR
      );
      ...
      createClientObjectivesByArray({
        ...
        .then((result) => {
          console.debug(
            `%chandleClickArray() createClientObjectivesByArray returned result`,
            COLOR
          );
        ...
        .finally(() => {
          ...
          console.debug(
            `%cSending message via L4LMC, message=${message}`,
            COLOR
          );
        ...
        .catch((error) => {
          this.error = error;
          console.error(
            `%chandleClickArray(): ERRORED ${JSON.stringify(error)}`,
            COLOR
          );
        });
    }
