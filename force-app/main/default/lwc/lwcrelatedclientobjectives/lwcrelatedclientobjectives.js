...
export default class lwcrelatedclientobjectives extends LightningElement {
  ...
  handleRowAction(event) {
    ...
    switch (actionName) {
      case "delete":
        ...
        console.debug(
          `%chandleRowAction: calling deleteRecord row.Id=${row.Id}`,
          COLOR
        );
        ...
        deleteRecord(row.Id)
          ...
          .catch((error) => {
            console.error(
              `%chandleRowAction: error in deleteRecord row.Id=${row.Id}`,
              COLOR
            );
      ...
      case "edit_details":
        ...
        console.debug(
          `%chandleRowAction: setting this.COrecordId=${row.Id}`,
          COLOR
        );
  ...
  refresh() {
    ...
    getClientObjectives({
      ...
      .then((result) => {
        console.debug(
          `%crefresh(): getClientObjectives returned ${JSON.stringify(result)}`,
          COLOR
        );
        this.clientobjectives = result;
        this.filterableObjectives = result;
        console.debug(
          `%crefresh(): this.clientObjectives= ${JSON.stringify(
  ...
  handleSave(event) {
    ...
    console.debug(
      `%chandleSave(): ${JSON.stringify(event.detail.draftValues)}`,
      COLOR
    );
  ...
  handleSearchKeyInput(event) {
    ...
    console.debug(
      `%chandleSearchKeyInput(): searchKey= ${searchKey}. this.filterableObjectives= ${JSON.stringify(
        this.filterableObjectives
      )}`,
      COLOR
    );
    ...
    console.debug(
      `%chandleSearchKeyInput(): this.clientobjectives=${JSON.stringify(
        this.clientobjectives
      )}`,
      COLOR
    );
  ...
  handleLMS(message) {
    ...
    console.debug(
      `%chandleLMS(): calling getClientObjectives, clientId=${this.recordId}`,
      COLOR
    );
    getClientObjectives({
      clientId: this.recordId
      ...
      .then((result) => {
        console.debug(
          `%chandleLMS(): returned result=${JSON.stringify(result)}`,
          COLOR
        );
        this.clientobjectives = result;
        this.filterableObjectives = result;
        console.debug(
          `%chandleLMS(): this.filterableObjectives=${JSON.stringify(
            this.filterableObjectives
          )}`,
          COLOR
        );
