    ...
    * */
    ...
    function tmpl$1i($api, $cmp, $slotset, $ctx) {
      ...
      return [api_custom_element("lightning-card", _lightningCard, {
        props: {
          "title": "Create Session Objectives in Bulk - V0.1.1 [Prototype]"
        },
        key: 2
      }, [api_element("br", {
        key: 3
        ...
        key: 7,
        on: {
          "click": _m1 || ($ctx._m1 = api_bind($cmp.handleClickCancel))
        }
      }, [])]), api_element("br", {
        key: 8
      }, []), api_text("DEBUG: Heres a list of candidate Objectives. Select one or more for creation of Session Objectives. Session record page refresh currently required to see results. Toast confirmation coming."), api_custom_element("lightning-datatable", _lightningDatatable, {
        props: {
          "keyField": "Id",
          "data": $cmp.objectives.data,
          "columns": $cmp.columns
        },
        key: 9,
        on: {
          "rowselection": _m2 || ($ctx._m2 = api_bind($cmp.getSelectedName))
        }
      }, [])]), api_text("DEBUG: End of Objectives List.")];
    }
    ...
    const apexInvoker$1 = lds.getApexInvoker("", "MBSessionObjectives", "createSessionObjectivesByArray", false);
    wireService.register(apexInvoker$1, lds.generateGetApexWireAdapter("", "MBSessionObjectives", "createSessionObjectivesByArray", false));

    const apexInvoker$2 = lds.getApexInvoker("", "MBSessionObjectives", "createSessionObjectives", false);
    wireService.register(apexInvoker$2, lds.generateGetApexWireAdapter("", "MBSessionObjectives", "createSessionObjectives", false));
    ...
    class Lwcpopulatesessionobjectives extends lwc.LightningElement {
      ...
      handleClickArray(event) {
        if (this.selectedRows) {
          ...
          apexInvoker$1({
            sessionid: this.recordId,
            jsonstr: this.selectedRows
          }).then(result => {
            console.log('RETURNED'); //this.recordsProcessed=result;
            //console.log(this.recordsProcessed + 'records processed.');
          }).then(() => {//console.log('Refreshing');
            //return refreshApex(this.objectives);
          }).then(() => {//this.showNotification('Success',this.recordsProcessed+ ' records processed.','success');
          }).catch(error => {
            this.error = error;
            console.log('ERRORED' + JSON.stringify(error));
          });
