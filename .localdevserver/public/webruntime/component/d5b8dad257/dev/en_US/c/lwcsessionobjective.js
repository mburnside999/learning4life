Webruntime.moduleRegistry.define('c/lwcsessionobjective', ['lwc', 'lightning/configProvider', 'force/lds', 'wire-service', 'lightning/navigation'], function (lwc, configProvider, lds, wireService, navigation) { 'use strict';
    ...
    * */
    ...
    function tmpl$1i($api, $cmp, $slotset, $ctx) {
      ...
      return [api_text("Hello world!"), api_custom_element("lightning-card", _lightningCard, {
        props: {
          "title": "Edit/Delete Session Objectives Inline - V1.0.3"
        },
        key: 2
      }, [api_element("input", {
        attrs: {
          "size": "40",
          "type": "search",
          "placeholder": "Search Program, Objective, SD or Comment"
        },
        key: 3,
        on: {
        ...
        on: {
          ...
          "save": _m5 || ($ctx._m5 = api_bind($cmp.handleSave)),
          "rowaction": _m6 || ($ctx._m6 = api_bind($cmp.handleRowAction))
        }
      }, [])])];
    }
    ...
    class Lwcsessionobjective extends lwc.LightningElement {
      constructor(...args) {
        super(...args);
        this.recordId = 'a3N2v000003Gr4VEAS';
      ...
      handleSearchKeyInput(event) {
        ...
        this.sessionObjectives = this.allObjectives.filter(so => so.Program__c.toLowerCase().includes(searchKey) || so.Objective_Name__c.toLowerCase().includes(searchKey) || so.SD__c.toLowerCase().includes(searchKey) || !(so.Comment__c == undefined) && so.Comment__c.toLowerCase().includes(searchKey));
      }
